...
> Если требуется видеть время в днях испольуйте сенсор `filter_time_left_days`.
...
Технические данные об общем времени работы бризера. Еденица измерения `s` (секунда), класс `duration`.

> [!NOTE]
> В Home Assistant будет отображаться в вррмате HH:MM:SS,
> без дней - два дня будет отображено как 48 часов. Это ограничение HA.
> Если требуется видеть время в днях испольуйте сенсор `work_time_days`.
...
> В Home Assistant будет отображаться в вррмате HH:MM:SS,
> без дней - два дня будет отображено как 48 часов. Это ограничение HA.
> Если требуется видеть время в днях испольуйте сенсор `fan_time_days`.
...
Для работы необходимо целевое значения CO2, минимальную и максимальную скорость вентиляции,
используя дополнительные параметры или соответствующие сущности number.
...
Обязательныее параметры:

- `co2`, идентификатор сенсора с датчиком CO2.

Дополнительные параметры:

- `setpoint`, целевое значение CO2. Детально см. в `number[auto_setpoint]`.
- `min_fan_speed`, минимальная скорость вентиляции. Детально см. в `number[auto_min_fan_speed]`.
- `max_fan_speed`, максимальная скорость вентиляции. Детально см. в `number[auto_max_fan_speed]`.
- `kp`, пропорцииональный коэффициент. По-умолчанию: 0.124675.
- `ti`, время интегрирования (в минутах). По-умолчанию: 8.
- `db`, зона нечувствительности. По-умолчанию: 50.
- `lambda`, автоматизация обрабатывающая значение CO2.
  Переменная `x` будет содержать текущее значение датчика CO2, вернуть необходимо скорость вентиляции.
...
Работает совместно с переключателем `switch[type=auto]`.
...
Не может быть меньше или равным `number[type=auto_min_fan_speed]`.
