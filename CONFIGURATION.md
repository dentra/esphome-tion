...
- `vport_id`, _[id]_: идентификатор `vport` подключенного к бризеру.
  По-умолчанию: определяется автоматически.
- `update_interval`, _[time]_: интервал опроса состояния бризера. По-умолчанию: 15s.
- `state_timeout`, _[time]_: время на прием ответа, после которого выставляется ошибка состояния если ответ не был получен.
  Должно быть меньше чем `update_interval`.
  По-умолчанию: 3s.
- `batch_timeout`, _[time]_: время сбора команд обновления.
  По-умолчанию: 200ms.
- `force_update`, _boolean_: поведение обновления состояний - только по изменению или всегда.
  По-умолчанию: False.
...
- `restore_state`, _boolean_: переодически сохранять состояние бризера в память, полезно при потере питания бризером.
  По-умолчанию: False.
- `enable_kiv`, _boolean_: только для модели Lite.
  Позволяет полностью остановить вентилятор и не закрывать заслонку.
  Работает только при выключенном обогревателе и внешней температуре воздуха больше 5°C.
  По-умолчанию: False.
...
- `lambda`, _[automation]_: автоматизация обрабатывающая значение CO2.
  ...
  Несовместим с `pi_controller`. **Важно**, сигналом изменения параметров будет 0 в качестве значения CO2, в этом случае возвращаемое значение не принципиально, оно будет отброшено.
...
tion:
 ...
  auto:
    co2: my_co2_sensor
    lambda: |-
      if (x == 0) return 0; // было изменение параметров
      if (x > 900) return 4;
      if (x > 800) return 3;
      if (x > 700) return 2;
      return 1;
...
>
> Для Lite с типом подключения UART получение актуального состояния невозможно, возможно только его изменение.
...
Изменяется при отрицательных температурах в зависимости от температуры наружного воздуха и возможно целевой температуры нагрева.

> [!IMPORTANT]
> Поддерживаемые модели: 4S, Lite.

> [!TIP]
> Алиасы: `fan_max_speed`
