 ...
 public:
  ...
  void log_state(const tion3s_state_t &st) {
    LOGD("fan_speed=%u, gate_position=%u, target_temperature=%d, heater_state=%s, power_state=%s, timer_state=%s, "
         "sound_state=%s, auto_state=%s, ma_connect=%s, save_bit=%s, ma_pairing=%s, preset_state=%s, "
         ...
         st.fan_speed, st.gate_position, st.target_temperature, ONOFF(st.system.heater_state),
         ONOFF(st.system.power_state), ONOFF(st.system.timer_state), ONOFF(st.system.sound_state),
         ONOFF(st.system.auto_state), ONOFF(st.system.ma_connect), ONOFF(st.system.save_bit),
         ONOFF(st.system.ma_pairing), ONOFF(st.system.preset_state), st.system.reserved, st.outdoor_temperature1,
         st.outdoor_temperature2, st.indoor_temperature, st.filter_time, st.hours, st.minutes, st.last_error,
         st.productivity, st.filter_days, st.firmware_version);
  }
};
...
bool test_api_3s(bool print) {
  ...
  api.state.system.heater_state = true;
  api.state.system.power_state = true;
  api.state.system.sound_state = true;
  api.state.target_temperature = 23;
  api.state.filter_time = 79;
  api.state.hours = 14;
  api.state.minutes = 45;
  api.state.gate_position = tion3s_state_t::OUTDOOR;
  api.write_state(api.state);
  ...
  api.state = {};
  test_check(res, api.read_data(from_hex("B3.10.21.17.0B.00.00.00.00.4F.00.0E.2D.00.00.00.00.00.00.5A")), true);
